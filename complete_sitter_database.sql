-- Complete Sitter Application Database Schema
...
CREATE TABLE IF NOT EXISTS public.messages (
    ...
    role TEXT DEFAULT 'message' CHECK (role IN ('message', 'invite', 'system')),
    status TEXT DEFAULT 'sent' CHECK (status IN ('sent', 'read', 'deleted')),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now()) NOT NULL
);
...
-- RLS Policies for child_group_members
...
-- Success message
SELECT 'Complete Sitter application database created successfully with all features!' as status; 