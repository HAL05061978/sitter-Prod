...
export default function SchedulePage() {
  ...
  const fetchRequestsAndResponses = async (userId: string) => {
    ...
    // Fetch open block invitations RECEIVED by the current user
    const { data: receivedOpenBlockInvitations } = await supabase
      .from("open_block_invitations")
      .select(`
        *,
        scheduled_care!open_block_id(
          *,
          children:child_id (
            id,
            full_name,
    ...
    // Fetch open block invitations SENT by the current user (only active ones)
    const { data: sentOpenBlockInvitations } = await supabase
      .from("open_block_invitations")
      .select(`
        *,
        scheduled_care!open_block_id(
      ...
      `)
      .eq("inviting_parent_id", userId)
      .eq("status", "active")  // Only show active invitations that need management
      .order("created_at", { ascending: false });


    
    // Convert RECEIVED open block invitations to care request format for display
    const receivedOpenBlockRequests = (receivedOpenBlockInvitations || []).map(invitation => {
      const scheduledCare = invitation.scheduled_care;
      
      if (!scheduledCare) {
        console.log('No scheduled care data found for invitation:', invitation.id);
        return null; // Skip this invitation
      }
    ...
    // Convert SENT open block invitations to care request format for editing
    // Group by block_time_id to show only one invitation per time block
    const groupedInvitations = new Map();
    (sentOpenBlockInvitations || []).forEach(invitation => {
      const blockTimeId = invitation.block_time_id;
      if (!groupedInvitations.has(blockTimeId)) {
        groupedInvitations.set(blockTimeId, invitation);
      }
    });
    
    const sentOpenBlockRequests = Array.from(groupedInvitations.values()).map(invitation => {
      const scheduledCare = invitation.scheduled_care;
      
      if (!scheduledCare) {
        console.log('No scheduled care data found for sent invitation:', invitation.id);
        return null; // Skip this invitation
      }
      
      return {
        id: `sent_${invitation.block_time_id}`, // Use block_time_id for unique identification
        block_time_id: invitation.block_time_id, // Store block time ID for editing all invitations
        group_id: scheduledCare.group_id,
        requester_id: userId, // Current user is the requester (inviting parent)
        child_id: scheduledCare.child_id,
        requested_date: scheduledCare.care_date,
        start_time: scheduledCare.start_time,
        end_time: scheduledCare.end_time,
        duration_minutes: scheduledCare.duration_minutes,
        notes: `Open block invitation sent - awaiting responses`,
        request_type: 'open_block_sent', // New type to distinguish sent invitations
        status: invitation.status,
        created_at: invitation.created_at,
        children: scheduledCare.children,
        // Open block specific fields
    ...
    // Combine regular requests with received and sent open block requests
    const allRequests = [...filteredRequests, ...receivedOpenBlockRequests, ...sentOpenBlockRequests];
  ...
  // Clean function for handling care requests
  ...
  // Handler for editing sent open block invitations
  const handleEditOpenBlockInvitation = async (request: CareRequest) => {
    if (!request.block_time_id) {
      alert('Error: Could not find block time ID');
      return;
    }
    
    console.log('Edit open block invitation for block_time_id:', request.block_time_id);
    
    // Pre-populate the open block form with current invitation data
    setOpenBlockForm({
      invitedParentIds: [], // We'll let them select parents again
      reciprocalTimeSlots: [{
        date: request.reciprocal_date || '',
    ...
    // Set the care block that's being edited
    setSelectedCareBlock({
      id: request.open_block_id || '',
    ...
    // Store the block_time_id for updating existing invitations
    setEditingBlockTimeId(request.block_time_id);
    
    // Open the modal
    setShowOpenBlockModal(true);
  };

  // Handler for canceling sent open block invitations  
  const handleCancelOpenBlockInvitation = async (request: CareRequest) => {
    if (!request.block_time_id) {
      alert('Error: Could not find block time ID');
      return;
    }
    
    if (!confirm('Are you sure you want to cancel all invitations for this time block?')) {
      return;
    }
    
    try {
      // Update ALL invitations with the same block_time_id to 'cancelled'
      const { error } = await supabase
        .from('open_block_invitations')
        .update({ status: 'cancelled' })
        .eq('block_time_id', request.block_time_id)
        .eq('inviting_parent_id', user?.id); // Safety check
        
      if (error) {
        console.error('Error canceling invitations:', error);
        alert('Error canceling invitations: ' + error.message);
        return;
      }
      
      alert('Open block invitations canceled successfully!');
      ...
      console.error('Error canceling invitations:', error);
      alert('Error canceling invitations. Please try again.');
    }
  };
  ...
  // Helper function to generate enhanced block text with providing parent's child info
  const getEnhancedBlockText = (providingParentName: string, childNames: string, block: any) => {
    let providingParentChild = '';
    
    // For open blocks and reciprocal arrangements, try to show the providing parent's child
    if (block.related_request_id) {
      const relatedRequest = requests.find(r => r.id === block.related_request_id);
      if (relatedRequest) {
        let providingChildId = '';
        
        if (relatedRequest.request_type === 'reciprocal' && relatedRequest.reciprocal_child_id) {
          providingChildId = relatedRequest.reciprocal_child_id;
        } else if (relatedRequest.request_type === 'open_block') {
          // For open blocks, the providing parent's child might be in the original reciprocal request
          providingChildId = relatedRequest.child_id;
        }
        
        if (providingChildId) {
          const providingChildName = getChildName(providingChildId, relatedRequest, block);
          if (providingChildName && providingChildName !== childNames) {
            providingParentChild = ` (with ${providingChildName})`;
          }
        }
      }
    }
    
    const enhancedText = `${providingParentName}${providingParentChild} providing care for ${childNames}`;
    
    // Debug: Log the enhancement
    if (providingParentChild) {
      console.log("🎯 Enhanced block text:", enhancedText);
      console.log("Original would have been:", `${providingParentName} providing care for ${childNames}`);
    }
  ...
  // Helper function to open a block for invitations
  ...
  const handleOpenBlockSubmit = async () => {
    const isEditing = editingBlockTimeId !== null;
    console.log("===", isEditing ? "EDITING" : "CREATING", "OPEN BLOCK INVITATION DEBUG ===");
    console.log("User creating invitations:", user?.id);
    console.log("Selected care block:", selectedCareBlock);
    console.log("Invited parent IDs:", openBlockForm.invitedParentIds);
    console.log("Reciprocal time slots:", openBlockForm.reciprocalTimeSlots);
    console.log("Editing block time ID:", editingBlockTimeId);
    ...
    try {
      ...
      // If editing, first cancel the existing invitations
      if (isEditing) {
        console.log("Canceling existing invitations for block_time_id:", editingBlockTimeId);
        const { error: cancelError } = await supabase
          .from('open_block_invitations')
          .update({ status: 'cancelled' })
          .eq('block_time_id', editingBlockTimeId)
          .eq('inviting_parent_id', user?.id);
          
        if (cancelError) {
          console.error('Error canceling existing invitations:', cancelError);
          alert('Error updating invitations: ' + cancelError.message);
          return;
        }
      }
      ...
      for (let timeSlotIndex = 0; timeSlotIndex < openBlockForm.reciprocalTimeSlots.length; timeSlotIndex++) {
        ...
        const blockTimeId = isEditing ? editingBlockTimeId : crypto.randomUUID(); // Reuse existing ID when editing
        
        console.log(`Creating invitations for time slot ${timeSlotIndex}:`, timeSlot);
        console.log(`Block time ID for this slot: ${blockTimeId}`);
        
        // Create invitation for each parent for this time slot
        for (const parentId of openBlockForm.invitedParentIds) {
          console.log(`Creating invitation for parent: ${parentId}`);
          
          // 🚨 DUPLICATE PREVENTION: Check if invitation already exists (skip when editing)
          if (!isEditing) {
            const { data: existingInvitation } = await supabase
              .from('open_block_invitations')
              .select('id, status')
              .eq('open_block_id', selectedCareBlock.id)
              .eq('invited_parent_id', parentId)
              .eq('reciprocal_date', timeSlot.date)
              .eq('reciprocal_start_time', timeSlot.startTime)
              .eq('reciprocal_end_time', timeSlot.endTime)
              .in('status', ['active', 'accepted']) // Don't prevent if previously declined/expired
              .single();
              
            if (existingInvitation) {
              console.log(`⚠️ Invitation already exists for parent ${parentId} on ${timeSlot.date} ${timeSlot.startTime}-${timeSlot.endTime}`);
              alert(`An open block invitation has already been sent to this parent for ${timeSlot.date} ${timeSlot.startTime}-${timeSlot.endTime}. Please edit the existing invitation instead.`);
              continue; // Skip this parent and move to next
            }
          }
          ...
          .insert({
              ...
              existing_block_id: selectedCareBlock.id, // Link to the existing care block for invitations to existing blocks
              ...
              inviting_parent_id: user.id, // NEW COLUMN: Parent B (who is inviting) - SIMPLE AND CLEAR
      ...
      setEditingBlockTimeId(null); // Clear editing state
      ...
      alert(isEditing 
        ? `Open block invitations updated successfully! ${invitations.length} invitation(s) sent with new times.`
        : `Open block invitations created successfully! ${invitations.length} invitation(s) sent. It's first-come-first-serve - the first parent to accept gets the slot.`);
    } catch (error) {
  ...
  const getAvailableParentsForOpenBlock = async (block?: ScheduledCare) => {
    ...
    console.log(`Target block: ${targetBlock.id}, Child ID: ${targetBlock.child_id}, Parent ID: ${targetBlock.parent_id}`);
    ...
    // If no original request (e.g., for "provided" blocks), we can still proceed
    // We'll just exclude the current user and the parent of the child being cared for
    ...
    console.log(`Group members found: ${groupMembers?.length || 0} for group ${targetBlock.group_id}`);
    console.log('Group members data:', groupMembers);
    
    if (!groupMembers || groupMembers.length === 0) {
      console.log('No group members found, returning empty array');
      return [];
    }
    
    const profileIds = groupMembers.map(member => member.profile_id);
    console.log(`Profile IDs to fetch: ${profileIds.join(', ')}`);
    ...
    console.log(`Profiles fetched: ${profilesData?.length || 0}`);
        ...
        // Get the child being cared for in this block and exclude their parent
    const { data: childData, error: childError } = await supabase
      .from('children')
      .select('parent_id, full_name')
      .eq('id', targetBlock.child_id)
      .single();
    ...
    if (!childData) {
      console.error('No child data found for child ID:', targetBlock.child_id);
      return [];
    }

    console.log(`Child being cared for: ${childData?.full_name || 'Unknown'} (${targetBlock.child_id}), Parent ID: ${childData?.parent_id || 'Unknown'}`);
    
    // Filter profiles to exclude only the current user and the parent of the child being cared for
    const availableParents = (profilesData || [])
      .filter(profile => {
        // Exclude current user and parent of the child being cared for
        if (profile.id === user?.id || 
            profile.id === childData.parent_id) {
          console.log(`Excluding profile ${profile.full_name} (${profile.id}) - reason: ${profile.id === user?.id ? 'current user' : 'parent of child being cared for'}`);
          return false;
        }
        
        console.log(`Including profile ${profile.full_name} (${profile.id})`);
        return true;
      });
    
    console.log(`Available parents: ${availableParents.length} out of ${profilesData?.length || 0} total profiles`);
  ...
  const acceptResponse = async (responseId: string, requestId: string) => {
    // Note: Open block requests are now handled via open_block_invitations, not care_requests
    // This function now only handles regular reciprocal requests
  ...
  // Fixed open block response handling
    const createOpenBlockCalendarBlocks = async (invitationId: string) => {
    console.log("=== OPEN BLOCK CALENDAR CREATION DEBUG ===");
    console.log("Creating calendar blocks for invitation ID:", invitationId);

    try {
      // Check what blocks exist BEFORE we create new ones
      const { data: beforeBlocks } = await supabase
        ...
        .gte("care_date", "2025-08-08") // Get recent blocks
        .order("care_date", { ascending: true });

      console.log("BEFORE creating blocks - existing scheduled_care entries:", beforeBlocks);
      ...
      // 2. Get the original care block
      const { data: originalBlock, error: blockError } = await supabase
        .from("scheduled_care")
        .select("*")
      ...
      const { data: newBlock, error: createError } = await supabase
        ...
        if (createError) {
        console.error("Error creating new care block:", createError);
          return;
        }

      console.log("Successfully created provided care block:", newBlock);

      // Note: We don't create a "needs care" block for the inviting parent
      // because they already have the original "provides care" block.
      // The invited parent accepting creates a reciprocal arrangement:
      // - Original block: Parent B provides care (already exists)
      // - New block: Parent C provides care (created above)
      ...
      console.log("AFTER creating blocks - all scheduled_care entries:", afterBlocks);

      // Compare before and after to see exactly what was created
      const newBlocks = afterBlocks?.filter(after => 
        !beforeBlocks?.some(before => before.id === after.id)
      ) || [];

      console.log("NEW BLOCKS CREATED by this function:", newBlocks);

      // Check for any blocks with null parent_id that might be causing "Unknown Parent"
      const { data: nullParentBlocks } = await supabase
        .from("scheduled_care")
        .select("*")
        .is("parent_id", null)
        .eq("care_date", invitation.reciprocal_date);

      if (nullParentBlocks && nullParentBlocks.length > 0) {
        console.warn("Found blocks with null parent_id:", nullParentBlocks);
      } else {
        console.log("No blocks with null parent_id found");
      }

      console.log("=== END OPEN BLOCK CALENDAR CREATION DEBUG ===");
    } catch (error) {
      console.error("Error creating calendar blocks:", error);
    }
  };

  const handleOpenBlockResponse = async (invitationId: string, response: 'accept' | 'decline', childId?: string) => {
    console.log("=== OPEN BLOCK RESPONSE HANDLING DEBUG ===");
    console.log("handleOpenBlockResponse called with:", { invitationId, response, childId });
    ...
    console.log("BEFORE acceptance - existing scheduled_care entries:", beforeAcceptBlocks);

    try {
      if (response === "accept") {

        // 1. Get the invitation details to understand the context
        const { data: invitation, error: fetchError } = await supabase
          .from("open_block_invitations")
          .select("*")
          .eq("id", invitationId)
        ...
        if (fetchError || !invitation) {
          console.error("Error fetching invitation:", fetchError);
          alert("Error processing open block invitation. Please try again.");
          return;
        }

        // 2. Update statuses for first-come-first-serve logic
        ...
        // 3. Expire all other invitations for the same invited_parent_id (same accepting user)
        const { error: expireSameUserError } = await supabase
          .from("open_block_invitations")
          .update({ status: "expired" })
          .eq("invited_parent_id", invitation.invited_parent_id)
          .neq("id", invitationId);

        if (expireSameUserError) {
          console.error("Error expiring other invitations for same user:", expireSameUserError);
        }

        // 4. Expire all other invitations for the same block_time_id (same time slot from same invitation batch)
        const { error: expireSameTimeError } = await supabase
          .from("open_block_invitations")
          .update({ status: "expired" })
          .eq("block_time_id", invitation.block_time_id)
          .neq("id", invitationId);

        if (expireSameTimeError) {
          console.error("Error expiring other invitations for same time:", expireSameTimeError);
        }

        // 5. Create the calendar blocks
        await createOpenBlockCalendarBlocks(invitationId);
      } else {
        // For decline, just update this invitation to declined
        const { error: updateError } = await supabase
          .from("open_block_invitations")
      ...
      // Check what blocks exist AFTER the entire acceptance process
      const { data: afterAcceptBlocks } = await supabase
        .from("scheduled_care")
        .select("id, parent_id, child_id, care_type, care_date, start_time, end_time, notes, related_request_id")
        .gte("care_date", "2025-08-08")
        .order("care_date", { ascending: true });

      console.log("AFTER acceptance - all scheduled_care entries:", afterAcceptBlocks);

      // Compare before and after to see exactly what was created during acceptance
      const newAcceptBlocks = afterAcceptBlocks?.filter(after => 
        !beforeAcceptBlocks?.some(before => before.id === after.id)
      ) || [];

      console.log("NEW BLOCKS CREATED during acceptance process:", newAcceptBlocks);
      ...
      console.log("=== END OPEN BLOCK RESPONSE HANDLING DEBUG ===");

      if (response === "accept") {
        alert("Open block invitation accepted successfully! Calendar blocks have been created.");
      } else {
        alert("Open block invitation declined.");
      }
    } catch (error) {
      console.error("Error handling open block response:", error);
      alert("Error processing open block invitation. Please try again.");
    }
  };
             ...
             <div key={request.id} className="bg-white rounded-lg border p-4">
               <div className="flex justify-between items-start">
                   ...
                   <div className="flex items-center gap-2 mb-2">
                     ...
                     <span className={`px-2 py-1 text-xs rounded-full ${
                       ...
                       {request.request_type === 'simple' ? 'Simple' :
                        ...
                        request.request_type === 'event' ? 'Event' :
                 ...
                 <div className="flex space-x-2 ml-4">
                   ...
                   {request.request_type === 'open_block_sent' ? (
                     // Special handling for SENT open block invitations
                     <>
                       <button
                         onClick={() => handleEditOpenBlockInvitation(request)}
                         className="px-3 py-1 text-sm bg-blue-600 text-white rounded hover:bg-blue-700"
                       >
                         Edit Times
                       </button>
                       <button
                         onClick={() => handleCancelOpenBlockInvitation(request)}
            ...
            <div className="space-y-4">
              {getBlocksForDate(selectedDateForDailyView).map(block => {
                ...
                if (isUserProviding) {
                  ...
                  console.log("Block causing 'Unknown Parent':", block);
                  console.log("Block related_request_id:", block.related_request_id);
                  
                  let originalRequest = findOriginalRequestForBlockSync(block);
                  console.log("Found original request in active requests:", originalRequest);
                  
                  // Note: We can't use await in render function, so we'll rely on existing data only
                  if (!originalRequest && block.related_request_id) {
                    console.log("Could not find original request for related_request_id:", block.related_request_id);
                    console.log("This might be a completed request not in active requests array");
                  }
                  ...
                  if (originalRequest) {
                    ...
                    console.log("Found accepted response:", acceptedResponse);
                    if (acceptedResponse) {
                      providingParentName = getParentName(acceptedResponse.responder_id);
                      console.log("Providing parent name resolved to:", providingParentName);
                    } else {
                      console.log("No accepted response found for request:", originalRequest.id);
                    }
                  } else {
                    console.log("No original request found for block with related_request_id:", block.related_request_id);
                  }
                  ...
                  if (block.care_type === 'needed') {
                    ...
                    // Note: Can't use await in render function
      ...
      {showOpenBlockModal && selectedCareBlock && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium">
                {editingBlockTimeId ? 'Edit Open Block Invitation' : 'Open Block Invitation'}
              </h3>
              <button
                onClick={() => {
                  setShowOpenBlockModal(false);
                ...
                <div>
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      console.log('=== OPEN BLOCK RESPONSE CHILD FILTERING DEBUG ===');
                      console.log('selectedOpenBlockInvitation:', selectedOpenBlockInvitation);
                      console.log('selectedOpenBlockInvitation keys:', Object.keys(selectedOpenBlockInvitation || {}));
                      
                      // Check different possible paths for scheduled_care data
                      const scheduledCareData = selectedOpenBlockInvitation?.scheduled_care || 
                                              selectedOpenBlockInvitation?.open_block_id || 
                                              selectedOpenBlockInvitation?.existing_block_id;
                      console.log('scheduledCareData:', scheduledCareData);
                      
                      // Try to get group_id from the invitation directly or from scheduled_care
                      let groupId = selectedOpenBlockInvitation?.group_id || 
                                   scheduledCareData?.group_id ||
                                   selectedOpenBlockInvitation?.scheduled_care?.group_id;
                      ...
                      // Check if the child exists in the base children array with proper status
                      const userChildren = children.filter(child => child.parent_id === user?.id);
                      console.log('userChildren in base array:', userChildren);
                      userChildren.forEach((child, index) => {
                        console.log(`Base child ${index}:`, child);
                        console.log(`  status in base: ${child.status}`);
                      });
                      
                      console.log('activeChildrenPerGroup:', activeChildrenPerGroup);
                      console.log('user?.id:', user?.id);
                      
                      if (!groupId) {
                        // Fallback to user's active children if no group ID available
                        console.log('No groupId - using fallback to active children');
                        const activeChildren = children.filter(child => child.status !== 'inactive');
                        console.log('activeChildren (fallback):', activeChildren);
                        return activeChildren.map(child => (
                          <option key={child.id} value={child.id}>{child.full_name}</option>
                        ));
                      }
                      
                      // Get active children for this specific group
                      const groupChildren = getActiveChildrenForGroup(groupId);
                      console.log('groupChildren for groupId', groupId, ':', groupChildren);
                      
                      // Debug each child in the group
                      groupChildren.forEach((child, index) => {
                        console.log(`Child ${index}:`, child);
                        console.log(`  parent_id: ${child.parent_id} (user: ${user?.id}) - match: ${child.parent_id === user?.id}`);
                        console.log(`  status: ${child.status} - active: ${child.status === 'active'}`);
                      });
                      
                      const userActiveGroupChildren = groupChildren.filter(child => 
                        child.parent_id === user?.id && child.status !== 'inactive'
                      );
                      console.log('userActiveGroupChildren (filtered):', userActiveGroupChildren);
                      
                      return userActiveGroupChildren.map(child => (
                        <option key={child.id} value={child.id}>{child.full_name}</option>
                      ));
                    })()}
                  </select>
