-- =====================================================
-- FIX CHILD_ID FOR HANGOUT/SLEEPOVER INVITATIONS
-- =====================================================
-- Hangout and sleepover invitations involve multiple children,
-- so child_id should be nullable for these request types

-- =====================================================
-- STEP 1: Make child_id nullable in care_requests
-- =====================================================

ALTER TABLE care_requests
ALTER COLUMN child_id DROP NOT NULL;

-- Add a comment explaining when child_id can be NULL
COMMENT ON COLUMN care_requests.child_id IS
'Child ID for single-child requests (reciprocal, open_block). NULL for multi-child requests (hangout, sleepover) where children are tracked in scheduled_care_children and care_responses.invited_child_id';

-- =====================================================
-- STEP 2: Update create_hangout_invitation function to set child_id to NULL
-- =====================================================
...
BEGIN
    ...
    INSERT INTO care_requests (
        group_id,
        requester_id,
        child_id,  -- Explicitly set to NULL for multi-child hangouts
        requested_date,
        start_time,
        end_time,
        end_date,
...
-- =====================================================
-- STEP 3: Update create_sleepover_invitation function similarly
-- =====================================================
...
BEGIN
    ...
    INSERT INTO care_requests (
        group_id,
        requester_id,
        child_id,  -- Explicitly set to NULL for multi-child sleepovers
        requested_date,
        start_time,
        end_time,
        end_date,
...
COMMENT ON FUNCTION create_hangout_invitation IS 'Creates a hangout invitation with multiple hosting and invited children. Sets child_id to NULL since children are tracked in scheduled_care_children and care_responses.';
COMMENT ON FUNCTION create_sleepover_invitation IS 'Creates a sleepover invitation with multiple hosting and invited children. Sets child_id to NULL since children are tracked in scheduled_care_children and care_responses.';
