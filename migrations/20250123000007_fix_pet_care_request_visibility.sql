-- =====================================================
-- FIX PET CARE REQUEST VISIBILITY
-- =====================================================
-- This fixes the issue where responders see requests
-- even after they've submitted their response.
--
-- The get_reciprocal_pet_care_requests function should
-- ONLY return requests with 'pending' status, not 'submitted'.
--
-- Once a responder submits their response, it should
-- disappear from their view and only appear for the
-- requester to accept/decline.
-- =====================================================
...
BEGIN
    ...
    AND pcr.status = 'pending'  -- FIXED: Only show pending requests (not submitted)
...
DO $$
BEGIN
    RAISE NOTICE '✅ get_reciprocal_pet_care_requests function updated!';
    RAISE NOTICE '✅ Now only returns pending requests (not submitted)';
    RAISE NOTICE '✅ Requests will disappear after responder submits their response';
END $$;
