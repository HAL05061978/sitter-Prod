-- =====================================================
-- FIX SLEEPOVER AND TIME CONSTRAINT
-- =====================================================
-- 1. Remove valid_care_time_range constraint (blocks overnight care)
-- 2. Update hangout function to support optional end_date for overnight hangouts
-- 3. Update sleepover invitation function to match hangout fixes

-- =====================================================
-- PART 1: Remove time range constraint
-- =====================================================

-- Drop the constraint that prevents overnight care blocks
ALTER TABLE scheduled_care
DROP CONSTRAINT IF EXISTS valid_care_time_range;

COMMENT ON TABLE scheduled_care IS 'Time constraint removed to allow overnight care (sleepovers, late hangouts). For multi-day events, end_date field indicates the actual end date.';

-- =====================================================
-- PART 2: Update create_hangout_invitation to support overnight
-- =====================================================

DROP FUNCTION IF EXISTS create_hangout_invitation(UUID, UUID, DATE, TIME, TIME, UUID[], UUID[], TEXT);
...
CREATE OR REPLACE FUNCTION create_hangout_invitation(
    ...
    p_end_date DATE DEFAULT NULL  -- Optional for overnight hangouts
)
RETURNS TABLE(
    success BOOLEAN,
    message TEXT,
...
BEGIN
    ...
    -- Determine effective end_date: if end_time < start_time, it goes to next day
    IF p_end_date IS NOT NULL THEN
        v_effective_end_date := p_end_date;
    ELSIF p_end_time < p_start_time THEN
        -- Overnight: add 1 day to care_date
        v_effective_end_date := p_care_date + INTERVAL '1 day';
    ELSE
        v_effective_end_date := NULL;  -- Same day
    END IF;
    ...
    VALUES (
        ...
        p_start_time,
        p_end_time,
        v_effective_end_date,  -- NULL if same day, next day if overnight
        'hangout',
        'hangout_invitation',
        'pending',
        p_notes
...
GRANT EXECUTE ON FUNCTION create_hangout_invitation(UUID, UUID, DATE, TIME, TIME, UUID[], UUID[], TEXT, DATE) TO authenticated;

COMMENT ON FUNCTION create_hangout_invitation IS 'Creates a hangout invitation with multiple hosting and invited children. Supports overnight hangouts by automatically detecting when end_time < start_time or using provided end_date.';

-- =====================================================
-- PART 3: Update create_sleepover_invitation to match hangout pattern
-- =====================================================

DROP FUNCTION IF EXISTS create_sleepover_invitation(UUID, UUID, DATE, TIME, DATE, TIME, UUID[], UUID[], TEXT);
...
COMMENT ON FUNCTION create_sleepover_invitation IS 'Creates a sleepover invitation with multiple hosting and invited children. Works with overnight time ranges by using end_date field.';
