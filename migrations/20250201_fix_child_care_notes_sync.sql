-- =====================================================
-- FIX: Child Care Notes Not Showing for Receiving Parent
-- =====================================================
-- Problem: Notes updated by provider don't appear on receiver's calendar
-- because get_scheduled_care_for_calendar just returns sc.notes directly
-- instead of pulling from the provider's block like it does for photos.
--
-- Solution: Update the function to pull notes from provider's block
-- for 'needed' care type, matching the existing photo_urls logic.
-- =====================================================
...
BEGIN
    ...
    SELECT
        ...
        sc.care_type::TEXT,
        sc.status::TEXT,
        -- FIXED: For 'needed' care, pull notes from the provider's block
        CASE
            WHEN sc.care_type = 'needed' THEN
                COALESCE(
                    (SELECT provider_care.notes
        ...
        -- Include action_type for rescheduled block display
        sc.action_type,
        ...
        CASE
            ...
            ELSE
                false
        END as is_host,
        -- Photo sharing logic - for 'needed' care, show photos from the provider's block
        CASE
...
COMMENT ON FUNCTION get_scheduled_care_for_calendar IS 'Returns scheduled care for calendar display with notes and photos synced from provider block to receiver';
...
BEGIN
    RAISE NOTICE '================================';
    RAISE NOTICE '✅ CHILD CARE NOTES SYNC FIX DEPLOYED!';
    RAISE NOTICE '================================';
    RAISE NOTICE 'Fixed:';
    RAISE NOTICE '1. Notes now pulled from provider block for receiving parents';
    RAISE NOTICE '2. Matches existing photo_urls sharing logic';
    RAISE NOTICE '3. Receiver will now see notes updated by provider';
    RAISE NOTICE '================================';
END $$;
