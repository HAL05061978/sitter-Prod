-- Fix children functions to work with any table structure
...
-- Function to get user's children for a specific group (works with any columns)
...
BEGIN
  ...
  SELECT 
    c.id,
    'Child ' || c.id::text as full_name,  -- Simple fallback that always works
    c.parent_id
  FROM children c
...
-- Function to get all children in a group (works with any columns)
...
-- Function to get children for a specific parent in a group (works with any columns)
