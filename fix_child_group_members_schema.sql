-- Fix child_group_members table schema
-- This script adds the missing added_by column if it doesn't exist
...
BEGIN
    ...
    -- Check if active column exists (this should NOT exist based on our schema)
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'child_group_members' 
        AND table_schema = 'public' 
        ...
        RAISE NOTICE 'WARNING: active column exists in child_group_members table - this should be removed';
    ELSE
        RAISE NOTICE 'active column does not exist (correct)';
    END IF;
END $$;

-- Update existing records to have added_by if the column was just added
-- This will set added_by to the first parent of each child
...
-- Success message
SELECT 'child_group_members table schema has been fixed!' as status; 