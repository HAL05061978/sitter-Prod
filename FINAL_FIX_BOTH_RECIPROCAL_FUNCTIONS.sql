-- FINAL FIX: Update both reciprocal care functions to work correctly
-- This fixes the "ambiguous column" error and adds 'pending' status support
...
BEGIN
    ...
    AND cr.response_type = 'pending'
    AND cr.status IN ('pending', 'submitted', 'accepted', 'declined')  -- ADDED 'pending'
    AND cq.request_type = 'reciprocal'
    ORDER BY cr.created_at DESC;
END;
$$;
