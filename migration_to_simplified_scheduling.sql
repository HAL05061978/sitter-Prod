-- Migration to Simplified Scheduling System
...
-- ============================================================================
-- STEP 9: MIGRATE EXISTING DATA (OPTIONAL - FOR TESTING)
-- ============================================================================
...
/*
-- Migrate existing babysitting requests to care_requests
INSERT INTO public.care_requests (
    id,
    group_id,
    requester_id,
...
SELECT 
    ...
    end_time,
    notes,
    'simple' as request_type,
    status,
    responder_id,
    response_notes,
    created_at,
    updated_at
FROM backup_babysitting_requests
WHERE status IN ('pending', 'accepted', 'declined');

-- Migrate existing scheduled blocks to scheduled_care
INSERT INTO public.scheduled_care (
    id,
    group_id,
    parent_id,
...
SELECT 
    ...
    created_at,
    updated_at
FROM backup_scheduled_blocks
WHERE status = 'confirmed';
*/

-- ============================================================================
-- STEP 10: CLEAN UP OLD TABLES (SAFE - AFTER TESTING)
-- ============================================================================

-- Comment out these DROP statements until you're confident the new system works
/*
...
-- Check if invitation_time_blocks exists and drop if it does
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'invitation_time_blocks') THEN
        DROP TABLE public.invitation_time_blocks;
        RAISE NOTICE 'Dropped invitation_time_blocks table';
    ELSE
        RAISE NOTICE 'invitation_time_blocks table does not exist, skipping drop';
    END IF;
END $$;
*/
...
SELECT '✅ Migration to simplified scheduling system completed!' as status;
...
SELECT 'Old tables preserved for now - uncomment DROP statements when ready' as cleanup_note; 