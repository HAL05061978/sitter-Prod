-- FIX CHILD_ID ISSUE IN CREATE OPEN BLOCK INVITATION
-- Fix the child_id reference issue without touching reciprocal functionality
...
DO $$
BEGIN
    RAISE NOTICE '=== FIXED CHILD_ID ISSUE ===';
    RAISE NOTICE '1. Fixed child_id reference in children table query';
    RAISE NOTICE '2. Changed child_id to id in SELECT statement';
    RAISE NOTICE '3. Added NULL check for child lookup';
    RAISE NOTICE '4. Function should now work without child_id column error';
