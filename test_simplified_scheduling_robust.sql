-- Test Simplified Scheduling System (ROBUST)
...
SELECT 'Checking new tables exist:' as info;
SELECT 
    table_name,
    CASE WHEN EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = table_name AND table_schema = 'public') 
         ...
         ELSE '❌ MISSING' 
    END as status
FROM (VALUES ('care_requests'), ('scheduled_care'), ('care_responses')) as t(table_name);
...
SELECT 'Available Groups:' as info, COUNT(*) as count FROM public.groups;
SELECT 'Available Profiles:' as info, COUNT(*) as count FROM public.profiles;
SELECT 'Available Children:' as info, COUNT(*) as count FROM public.children;

-- Show sample data (just IDs to avoid column issues)
SELECT 'Sample Groups:' as info;
SELECT id FROM public.groups LIMIT 3;

SELECT 'Sample Profiles:' as info;
SELECT id FROM public.profiles LIMIT 3;

SELECT 'Sample Children:' as info;
SELECT id, parent_id FROM public.children LIMIT 3;

-- ============================================================================
-- STEP 3: TEST SIMPLE REQUEST FLOW
-- ============================================================================
...
BEGIN
    ...
    IF v_group_id IS NULL THEN
        RAISE NOTICE '❌ No groups available';
        v_test_data_ok := false;
    END IF;
    
    IF v_requester_id IS NULL THEN
        RAISE NOTICE '❌ No profiles available';
        v_test_data_ok := false;
    END IF;
    
    IF v_child_id IS NULL THEN
        RAISE NOTICE '❌ No children available for requester';
        v_test_data_ok := false;
    END IF;
    
    IF NOT v_test_data_ok THEN
        RAISE NOTICE '❌ Missing test data - cannot proceed with simple request test';
        RETURN;
    END IF;
    ...
    -- Test accepting the request (if function exists)
    BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE '⚠️  accept_care_request function not available or failed: %', SQLERRM;
    END;
...
-- ============================================================================
-- STEP 4: TEST RECIPROCAL REQUEST FLOW
-- ============================================================================
...
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE '⚠️  create_open_block_requests function not available or failed: %', SQLERRM;
    END;
...
-- ============================================================================
-- STEP 6: TEST EVENT REQUEST FLOW
-- ============================================================================
...
BEGIN
    ...
    BEGIN
        ...
        END IF;
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE '⚠️  edit_scheduled_care function not available or failed: %', SQLERRM;
    END;
...
-- ============================================================================
-- STEP 8: TEST TIME CONFLICT CHECKING
-- ============================================================================
...
BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE '⚠️  check_care_time_conflicts function not available or failed: %', SQLERRM;
    END;
