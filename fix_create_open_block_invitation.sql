-- FIX CREATE OPEN BLOCK INVITATION FUNCTION
-- This fixes the issue where times are not being stored correctly in care_requests table
...
DO $$
BEGIN
    RAISE NOTICE '=== FIXED CREATE OPEN BLOCK INVITATION FUNCTION ===';
    RAISE NOTICE '1. FIXED: requested_date/start_time/end_time now store ORIGINAL block times (the block being opened)';
    RAISE NOTICE '2. FIXED: reciprocal_date/start_time/end_time now store RECIPROCAL times (the times being offered)';
    RAISE NOTICE '3. FIXED: Accepting party fields (reciprocal_parent_id, etc.) are NULL until accepted';
    RAISE NOTICE '4. FIXED: Proper time storage for calendar building';
