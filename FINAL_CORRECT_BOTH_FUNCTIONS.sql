-- FINAL CORRECT FIX: Swap the logic in both functions to match their names
-- get_reciprocal_care_requests should return requests I MADE (where I'm requester)
-- get_reciprocal_care_responses should return invitations I RECEIVED (where I'm responder)

-- =====================================================
-- Fix 1: get_reciprocal_care_requests
-- Should return: Requests I MADE where others are responding
-- =====================================================
...
BEGIN
    ...
    WHERE cq.requester_id = parent_id  -- Changed: I am the REQUESTER
...
-- Should return: Invitations I RECEIVED (where I need to respond)
-- =====================================================
...
BEGIN
    ...
    WHERE cr.responder_id = parent_id  -- Changed: I am the RESPONDER
    AND cr.response_type = 'pending'
    AND cr.status IN ('pending', 'submitted', 'accepted', 'declined')
    AND cq.request_type = 'reciprocal'
    AND cq.requester_id != parent_id  -- Don't show my own requests
    ORDER BY cr.created_at DESC;
END;
$$;
