-- FIX CALENDAR PROVIDER LOGIC V2
-- Completely rewritten get_scheduled_care_for_calendar function to fix column ambiguity
-- and correctly determine providing_parent_name based on care_type
...
DO $$
BEGIN
    RAISE NOTICE '=== FIX CALENDAR PROVIDER LOGIC V2 ===';
    RAISE NOTICE '1. Completely rewritten get_scheduled_care_for_calendar function';
    RAISE NOTICE '2. FIXED: Column ambiguity issue resolved with unique table aliases';
    RAISE NOTICE '3. FIXED: providing_parent_name correctly determined based on care_type';
    RAISE NOTICE '4. For care_type = provided: shows logged-in user as provider';
    RAISE NOTICE '5. For care_type = needed: shows actual provider from corresponding block';
