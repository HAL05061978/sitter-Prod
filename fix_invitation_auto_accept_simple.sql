-- Simple Fix for Invitation Auto-Accept Bug
-- This uses ONLY existing tables - no new tables or columns
...
BEGIN
    ...
    IF EXISTS (
        ...
        AND response_notes LIKE '%Invitation: ' || p_invitation_id || '%'
    ) THEN
    ...
    INSERT INTO care_responses (
        ...
        p_accepting_user_id,
        'accept',
        COALESCE(p_notes, '') || ' | Time Block: ' || p_selected_time_block_index || ' | Invitation: ' || p_invitation_id,
        'pending',
        p_selected_child_id
    );
...
BEGIN
    ...
    -- Extract invitation ID from response_notes
    v_invitation_id := (
        SELECT SUBSTRING(response_notes FROM 'Invitation: ([a-f0-9-]+)')
        FROM care_responses 
        WHERE id = p_response_id
    );
    
    IF v_invitation_id IS NULL THEN
        RAISE EXCEPTION 'Could not extract invitation ID from response';
    END IF;
    
    -- Get the invitation details
    SELECT * INTO v_invitation FROM group_invitations WHERE id = v_invitation_id::UUID;
    ...
    SET status = 'declined'
    WHERE request_id = v_request.id 
    AND response_notes LIKE '%Invitation: ' || v_invitation_id || '%'
...
-- ============================================================================
-- STEP 3: Create function to get responses for an invitation
-- ============================================================================
...
BEGIN
    ...
    WHERE cr.response_notes LIKE '%Invitation: ' || p_invitation_id || '%'
...
SELECT 'Invitation flow has been fixed to use existing tables without creating new ones.' as note; 