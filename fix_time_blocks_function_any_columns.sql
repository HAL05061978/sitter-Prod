...
BEGIN
  RETURN QUERY
  SELECT 
    COALESCE(itb.block_index, 0) as block_index,
    itb.block_date,
    itb.block_start_time,
    itb.block_end_time,
    COALESCE(itb.block_duration_minutes, 0) as block_duration_minutes,
    true as is_available  -- Always assume available for now
  FROM invitation_time_blocks itb
  WHERE itb.invitation_id = p_invitation_id
  ORDER BY COALESCE(itb.block_index, 0);
END;
$$ LANGUAGE plpgsql;
