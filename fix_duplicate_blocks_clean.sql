...
BEGIN
    ...
    RAISE NOTICE 'Checking reciprocal condition: response_type=%, request_type=%, reciprocal_date=%', 
        v_response.response_type, v_request.request_type, v_response.reciprocal_date;
    ...
    IF (v_response.response_type = 'accept' OR (v_response.response_type = 'pending' AND v_request.request_type = 'reciprocal')) 
       AND v_response.reciprocal_date IS NOT NULL THEN
        RAISE NOTICE 'Creating reciprocal blocks: Parent B needs care, Parent A provides care';
    ...
    ELSE
        RAISE NOTICE 'Reciprocal condition NOT met: response_type=%, request_type=%, reciprocal_date=%', 
            v_response.response_type, v_request.request_type, v_response.reciprocal_date;
    END IF;
